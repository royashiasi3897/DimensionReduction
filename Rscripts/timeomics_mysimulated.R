...
profile.filtered <- sim.data

conflicts_prefer(mixOmics::pca)
# run pca
pca.res <- pca(X = profile.filtered, ncomp = 50, scale=FALSE, center=FALSE)

# tuning ncomp
pca.ncomp <- getNcomp(pca.res, max.ncomp = 50, X = profile.filtered,
